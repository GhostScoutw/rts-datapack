...
summon wandering_trader ~ ~ ~ {Tags:["troop","mage","init","offensive"],NoAI:true,\
    CustomName:"Mage"\
}
...
execute as @n[tag=troop,tag=init] \
    store result score @s generic.attack_cooldown run scoreboard players set @s generic.attack_cooldown.max 80
