...
summon villager ~ ~ ~ {Tags:["troop","archer","init","offensive"],NoAI:true,\
...
execute as @n[tag=troop,tag=init] \
    store result score @s generic.attack_cooldown run scoreboard players set @s generic.attack_cooldown.max 60
execute as @n[tag=troop,tag=init] run attribute @s max_health base set 20
execute as @n[tag=troop,tag=init] \
